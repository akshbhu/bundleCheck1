...
class GraphQLApi extends aws_appsync_alpha_1.GraphqlApiBase {
    constructor(scope, id, props) {
        var _a, _b;
        super(scope, id);
        this.authorizationConfig = {
            defaultAuthorization: { authorizationType: aws_appsync_alpha_1.AuthorizationType.API_KEY },
        ...
        const hasApiKey = modes.some(mode => mode.authorizationType === aws_appsync_alpha_1.AuthorizationType.API_KEY);
        if (props.createApiKey && hasApiKey) {
            const config = (_b = modes.find((mode) => mode.authorizationType === aws_appsync_alpha_1.AuthorizationType.API_KEY && mode.apiKeyConfig)) === null || _b === void 0 ? void 0 : _b.apiKeyConfig;
    ...
    validateAuthorizationProps(modes) {
        modes.forEach(mode => {
            if (mode.authorizationType === aws_appsync_alpha_1.AuthorizationType.OIDC && !mode.openIdConnectConfig) {
                throw new Error('Missing default OIDC Configuration');
            }
            if (mode.authorizationType === aws_appsync_alpha_1.AuthorizationType.USER_POOL && !mode.userPoolConfig) {
                throw new Error('Missing default OIDC Configuration');
            }
        });
        if (modes.filter(mode => mode.authorizationType === aws_appsync_alpha_1.AuthorizationType.API_KEY).length > 1) {
            throw new Error("You can't duplicate API_KEY configuration. See https://docs.aws.amazon.com/appsync/latest/devguide/security.html");
        }
        if (modes.filter(mode => mode.authorizationType === aws_appsync_alpha_1.AuthorizationType.IAM).length > 1) {
    ...
    setupUserPoolConfig(config) {
        ...
        return {
            ...
            defaultAction: config.defaultAction || aws_appsync_alpha_1.UserPoolDefaultAction.ALLOW,
        };
    }
