...
async function console(context, service) {
    ...
    const { openConsole } = await Promise.resolve().then(() => __importStar(require(serviceWalkthroughSrc)));
    if (!openConsole) {
...
async function updateContainerResource(context, category, service, apiType) {
    ...
    (0, containers_handler_1.updateResource)(updateWalkthroughPromise, context, category);
}
